...
public class CommandLine {
    ...
    public void start() {
        ...
        IntStream.range(0, crawledCourses.size())
                ...
                .forEach(course -> {
                    File courseDir = new File(dest, (++n[0]) + ". " + course.getTitle().replaceAll("[\\\\/:*?\"<>|]", "").trim());
                    ...
                    categories.forEach(category -> {
                        ...
                        File categoryDir = new File(courseDir, (++categoriesN[0]) + ". " + category.getName().replaceAll("[\\\\/:*?\"<>|]", "").trim());
                        ...
                        category.getActivities().forEach(activity -> {
                            ...
                            if(activity.getIndent() > lastIndent[0])
                                currentNumbers.add(0);
                            if(activity.getIndent() < lastIndent[0])
                                currentNumbers.removeLast();
                            lastIndent[0] = activity.getIndent();
